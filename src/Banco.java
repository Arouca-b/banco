...
//Autor Breno Arouca
//Última atualiazação: 11/05/2024
...
public class Banco {
    ...
    public static void main(String[] args) {
        ...
        ArrayList<IComunicable> destinatario = new ArrayList<>();
        ...
        do {
            ...
            switch (op) {
                case 1:
                    ...
                    ddd = inputText.nextLine();

                    System.out.print("Telefone: ");
                    telefone = inputText.nextLine();

                    System.out.print("Email: ");
                    email = inputText.nextLine();
                    ...
                    newConta = new Conta(nome,cpf, ddd, telefone, email, conta, agencia, senha);
                    listaClientes.add(newConta); 
                    
                    clear();
                    listaClientes.get(listaClientes.size() - 1).imprimir();
                    pause();
                    mensagem();
                    destinatario.add(listaClientes.get(Conta.getQuantidade_contas()-1));
                    break;
                ...
                case 0:
                    for (IComunicable iComunicable : destinatario) {
                        iComunicable.Enviar();
                    }
                    System.out.println("\n\nVOLTE SEMPRE\n\n");
                    break;
    ...
    public static void realizarPix(ArrayList<Conta> listaClientes, Conta cliente){
        ...
        for(int i = 0; i < Conta.getQuantidade_contas() && (!feito);i++){
            if (listaClientes.get(i).cpf().equals(chavePix) && chavePix!=cliente.cpf()) {
                System.out.print("Valor: R$ ");
                valor = inputValue.nextDouble();
                if (cliente.validaSaldo(valor)) {
                    cliente.pix(valor, cliente.nome(), listaClientes.get(i));
                    mensagem();
                    feito = true;
                }else{
                    System.out.println("\n\nSALDO INDISPONÍVEL!\n\n");
